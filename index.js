...
async function fetchMarketData() {
  try {
    ...
    const _qdata = q?.data || {};
    const d = _qdata['NSE_INDEX|Nifty 50'] || _qdata['NSE_INDEX:Nifty 50'] || _qdata['NSE_INDEX: Nifty 50'] || _qdata[Object.keys(_qdata)[0]];
    if (d) {
      state.niftySpot = d.last_price || 0;
      if (d.ohlc?.open) state.niftyOpen = d.ohlc.open;
    }

    // ── Intraday candles for VWAP + indicators ──
    const cResp = await upstox('GET', '/v3/historical-candle/intraday/NSE_INDEX%7CNifty%2050/minutes/15');
    ...
    if (state.candles.length > 0) {
      ...
      // Today open from first candle
      if (!state.niftyOpen && state.candles.length > 0) {
        state.niftyOpen = state.candles[0][1];
      }
      log('INFO', 'Market data: NIFTY ₹' + state.niftySpot + ' VWAP ₹' + state.vwap + ' Candles: ' + state.candles.length);
    } else {
      // Candles empty — use spot as VWAP fallback
      if (state.niftySpot > 0) {
        state.vwap = state.niftySpot;
        log('WARN', 'No candles — using spot as VWAP. NIFTY ₹' + state.niftySpot);
      }
    }
    ...
    const daily = await upstox('GET',
      '/v2/historical-candle/NSE_INDEX%7CNifty%2050/day/' + toDate + '/' + fromDate);
    const dCandles = daily?.data?.candles || [];
    if (dCandles.length >= 2) {
      const prev = dCandles[1]; // previous day
      state.pdHigh = prev[2];
      state.pdLow  = prev[3];
    }
...
async function getOptionKey(strike, direction, expiryDate) {
  try {
    ...
    // If empty, try fetching all expiries and find nearest
    if (contracts.length === 0) {
      log('WARN', 'No contracts for ' + expiryDate + ' — fetching all expiries');
      const allResp = await upstox('GET', '/v2/option/contract?instrument_key=NSE_INDEX%7CNifty%2050');
      const allContracts = allResp?.data || [];
      // Get nearest expiry
      const dates = [...new Set(allContracts.map(c => c.expiry))].sort();
      const today = new Date().toISOString().slice(0,10);
      const nearest = dates.find(d => d >= today) || dates[0];
      if (nearest) {
        log('INFO', 'Using nearest expiry: ' + nearest);
        contracts = allContracts.filter(c => c.expiry === nearest);
      }
    }
    ...
    // If exact strike not found, find nearest available strike
    if (!match) {
      const sameDir = contracts.filter(c => c.instrument_type === direction);
      const nearest = sameDir.sort((a,b) =>
        Math.abs(Number(a.strike_price)-strike) - Math.abs(Number(b.strike_price)-strike)
      )[0];
      if (nearest) {
        log('WARN', 'Exact strike ' + strike + ' not found, using nearest: ' + nearest.strike_price);
        return nearest;
      }
    }
    return match || null;
  } catch(e) { log('ERROR', 'getOptionKey: ' + e.message); return null; }
}
...
async function getNextExpiry() {
  // NIFTY weekly expiry is Monday (day 1)
  // Use today if today is Monday and before 3:30 PM
  // Otherwise find next Monday
  const now = new Date();
  const mins = now.getHours() * 60 + now.getMinutes();
  let d = new Date(now);
  d.setHours(0,0,0,0);
  // If today is Monday and market not yet closed
  if (d.getDay() === 1 && mins < 930) {
...
async function getRealBalance() {
  try {
    const resp = await upstox('GET', '/v2/user/get-funds-and-margin');
