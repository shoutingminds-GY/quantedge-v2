...
async function fetchMarketData() {
  try {
    // NIFTY spot + OHLC
    const q = await upstox('GET', '/v2/market-quote/quotes?instrument_key=NSE_INDEX|Nifty 50');
    const _qdata = q?.data || {};const d = _qdata['NSE_INDEX|Nifty 50'] || _qdata['NSE_INDEX:Nifty 50'] || _qdata['NSE_INDEX: Nifty 50'] || _qdata[Object.keys(_qdata)[0]];
    if (d) {
    ...
    const cResp = await upstox('GET',
      '/v2/historical-candle/intraday/NSE_INDEX|Nifty 50/15minute');
    ...
    const daily = await upstox('GET',
      '/v2/historical-candle/NSE_INDEX|Nifty 50/day/2');
...
async function getOptionKey(strike, direction, expiryDate) {
  try {
    ...
    const resp = await upstox('GET',
      '/v2/option/contract?instrument_key=NSE_INDEX|Nifty 50&expiry_date=' + expiryDate);
    ...
    if (contracts.length === 0) {
      ...
      const allResp = await upstox('GET', '/v2/option/contract?instrument_key=NSE_INDEX|Nifty 50');
